...
namespace Moira.ApiClient.Model
{
    ...
    public partial class DtoNotifierStateForSource : IValidatableObject
    {
        ...
        [JsonConstructor]
        public DtoNotifierStateForSource(string actor, string clusterId, string state, string triggerSource, Option<string> message = default)
        {
            Actor = actor;
            ClusterId = clusterId;
            State = state;
    ...
    public class DtoNotifierStateForSourceJsonConverter : JsonConverter<DtoNotifierStateForSource>
    {
        ...
        public override DtoNotifierStateForSource Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!actor.IsSet)
                throw new ArgumentException("Property is required for class DtoNotifierStateForSource.", nameof(actor));

            if (!clusterId.IsSet)
                throw new ArgumentException("Property is required for class DtoNotifierStateForSource.", nameof(clusterId));

            if (!state.IsSet)
                throw new ArgumentException("Property is required for class DtoNotifierStateForSource.", nameof(state));

            if (!triggerSource.IsSet)
                throw new ArgumentException("Property is required for class DtoNotifierStateForSource.", nameof(triggerSource));
            ...
            return new DtoNotifierStateForSource(actor.Value, clusterId.Value, state.Value, triggerSource.Value, message);
        }
        ...
        {
            if (dtoNotifierStateForSource.Actor == null)
            ...
            if (dtoNotifierStateForSource.ClusterId == null)
            ...
            if (dtoNotifierStateForSource.State == null)
            ...
            if (dtoNotifierStateForSource.TriggerSource == null)
